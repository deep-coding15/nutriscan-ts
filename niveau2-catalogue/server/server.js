...
const app  = express();
const PORT = 3002;
const FETCH_TIMEOUT = 3000; // ms avant bascule offline
...
function rechercheOffline(query) {
    ...
    const produits = datasetOffline.products
        .filter(p => p.product_name.toLowerCase().includes(q))
        .map(p => ({
            ...p,
            // Convertit "France,Suisse" → ["France", "Suisse"] pour coller au format online
            countries_tags: p.countries
                ? p.countries.split(',').map(c => c.trim())
                : [],
        }));
...
app.get('/api/search', async (req, res) => {
    ...
    try {
        ...
        console.log(`[online] ← ${response.status} "${query}"`);
